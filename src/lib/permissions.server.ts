 ...
 *  unauthenticated. Staff (coach/booster) always allowed. An athlete may
 *  only view their own profile. A parent may only view athlete(s) they are
 *  actually linked to (legacy single athlete_id + team_member_athletes for
 *  multi-child support — see getLinkedAthleteIds). Everyone else,
 *  including a different athlete's own account, is denied — mirroring the
 *  UI's per-row click gating below is not sufficient on its own; this is
