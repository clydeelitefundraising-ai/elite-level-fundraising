// Phase 2C: behavior of the native push registration flow, exercised with an
// injected plugin (no Capacitor runtime, no jsdom). Tokens here are obviously
// fake strings.
...
function fakePlugin(opts: { initial: string; afterRequest?: string; channelFails?: boolean }) {
  ...
  const plugin = {
    ...
    async checkPermissions() { calls.push("checkPermissions"); return { receive: opts.initial }; },
...
async function run(platform: "ios" | "android", plugin: PushPluginLike, extra: {
  ...
  const remove = await startNativePushRegistration({
    platform,
    plugin,
...
// ── iOS: identical to the pre-Android sequence ───────────────────────────────

test("iOS: listeners, then checkPermissions, requestPermissions on 'prompt', then register -- no channel", async () => {
  const f = fakePlugin({ initial: "prompt", afterRequest: "granted" });
  await run("ios", f.plugin);
  assert.deepEqual(f.calls, [...LISTENERS, "checkPermissions", "requestPermissions", "register"]);
});

test("iOS: already granted -> no prompt, register", async () => {
  const f = fakePlugin({ initial: "granted" });
  await run("ios", f.plugin);
  assert.deepEqual(f.calls, [...LISTENERS, "checkPermissions", "register"]);
});

test("iOS: denied -> no prompt and no register", async () => {
  const f = fakePlugin({ initial: "denied" });
  await run("ios", f.plugin);
  assert.deepEqual(f.calls, [...LISTENERS, "checkPermissions"]);
});
...
test("iOS: ignores the Android once-per-session prompt flag (previous behavior preserved)", async () => {
  const f = fakePlugin({ initial: "prompt", afterRequest: "granted" });
  await run("ios", f.plugin, { session: memorySession(true) });
  assert.ok(f.calls.includes("requestPermissions"));
  assert.ok(f.calls.includes("register"));
});
...
test("iOS token -> registerToken('ios', token)", async () => {
  const f = fakePlugin({ initial: "granted" });
  const { registered } = await run("ios", f.plugin);
...
test("Android 12 and below (plugin reports 'granted'): no prompt, channel then register", async () => {
  const f = fakePlugin({ initial: "granted" });
  await run("android", f.plugin);
  assert.deepEqual(f.calls, [...LISTENERS, "checkPermissions", "createChannel", "register"]);
});

test("Android 13+ 'prompt' -> request once -> granted -> channel then register", async () => {
  const f = fakePlugin({ initial: "prompt", afterRequest: "granted" });
  await run("android", f.plugin);
  assert.deepEqual(f.calls, [...LISTENERS, "checkPermissions", "requestPermissions", "createChannel", "register"]);
});
...
test("Android: the prompt is shown at most once per app session (no nagging on later mounts)", async () => {
  ...
  await run("android", first.plugin, { session });
  ...
  await run("android", second.plugin, { session });
...
test("Android: a later mount registers once the user granted permission in system settings", async () => {
  ...
  const f = fakePlugin({ initial: "granted" });
  await run("android", f.plugin, { session });
  assert.ok(f.calls.includes("register"));
});

test("Android: effect torn down while listeners attached -> listeners removed, no permission or register calls", async () => {
  const f = fakePlugin({ initial: "prompt", afterRequest: "granted" });
  await run("android", f.plugin, { cancelled: true });
...
test("remove() detaches all three listeners", async () => {
  const f = fakePlugin({ initial: "granted" });
  const { remove } = await run("android", f.plugin);
  remove();
...
test("tap: safe same-origin paths navigate (both platforms)", async () => {
  for (const platform of ["ios", "android"] as const) {
    ...
    await run(platform, f.plugin, { navigated });
...
test("tap: unsafe or missing urls are ignored", async () => {
  const f = fakePlugin({ initial: "denied" });
  const navigated: string[] = [];
  await run("android", f.plugin, { navigated });
  const onTap = f.handlers["pushNotificationActionPerformed"] as (a: unknown) => void;
  for (const url of [
...
test("no console output at all while registering, rotating and routing (tokens never logged)", async () => {
