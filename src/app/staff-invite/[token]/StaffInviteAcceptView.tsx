...
type Props =
  | { status: "invalid" | "expired" | "used" | "revoked"; token: string }
  | {
      status:             "valid";
      token:              string;
      inviteeName:        string;
...
function roleLabel(role: "assistant_coach" | "booster"): string {
  return role === "assistant_coach" ? "Assistant Coach" : "Booster";
}
...
export default function StaffInviteAcceptView(props: Props) {
  ...
  if (props.status === "invalid") {
    return <ErrorCard title="Invalid Link" message="This invitation link is not valid. Please contact your Head Coach for a new one." />;
  }
  if (props.status === "expired") {
    return <ErrorCard title="Invitation Expired" message="This invitation has expired (links are valid for 24 hours). Please ask your Head Coach to resend it." />;
  }
  if (props.status === "used") {
    return <ErrorCard title="Invitation Already Used" message="This invitation has already been accepted. If you need access, ask your Head Coach to send a new invite." />;
  }
  if (props.status === "revoked") {
    return <ErrorCard title="Invitation Revoked" message="This invitation is no longer valid. Please contact your Head Coach if you still need access." />;
  }

  if (props.status !== "valid") return null;
  const { token, inviteeName, email, role, teamName, hasExistingAccount } = props;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setBusy(true);
    try {
      const res  = await fetch("/api/auth/staff-invite-accept", {
        method:  "POST",
        headers: { "Content-Type": "application/json" },
        body:    JSON.stringify({ token, password: pw || undefined }),
      });
      const data = await res.json();
      if (!res.ok) { setError(data.error ?? "This invitation could not be accepted."); return; }
      ...
      return (
        <Shell>
          <div style={CARD_STYLE}>
            ...
            <h1 style={{ margin: 0, fontSize: "1.5rem", fontWeight: 800, color: "#0b1e3d" }}>Team Added</h1>
    ...
    <Shell>
      <form onSubmit={handleSubmit} style={CARD_STYLE}>
        <h1 style={{ margin: 0, fontSize: "1.5rem", fontWeight: 800, color: "#0b1e3d", letterSpacing: "-.02em" }}>
          {hasExistingAccount ? "Join This Team" : "Set Your Password"}
        </h1>
        ...
        <div style={{ background: "#f0f9ff", border: "1px solid #bae6fd", borderRadius: ".65rem", padding: ".875rem 1rem", fontSize: ".875rem", color: "#0369a1", lineHeight: 1.5 }}>
          <strong>{inviteeName}</strong> &middot; {email}
          <div style={{ marginTop: ".35rem" }}>
            Invited as <strong>{roleLabel(role)}</strong> for <strong>{teamName}</strong>
          </div>
        </div>
        ...
        {hasExistingAccount ? (
          <p style={{ margin: 0, fontSize: ".95rem", color: "#374151", lineHeight: 1.6 }}>
            Your email is already registered with ELF. Click below to add this team to your existing account.
