...
export async function PATCH(req: NextRequest, { params }: RouteContext) {
  ...
  logAuditEvent({
    ...
    action: "clearance_resource.updated",
    entity_type: "clearance_resource",
    entity_id: id,
    campaign_slug: slug,
    summary: `Updated Clearance resource "${result.resource.title}" on ${slug}`,
    ip_address: ipOf(req),
...
export async function DELETE(req: NextRequest, { params }: RouteContext) {
  ...
  logAuditEvent({
    actor: toAuditActor(actor),
    action: "clearance_resource.deleted",
    entity_type: "clearance_resource",
    entity_id: id,
    campaign_slug: slug,
    summary: `Deleted Clearance resource ${id} from ${slug}`,
    ip_address: ipOf(req),
    user_agent: req.headers.get("user-agent"),
  });
