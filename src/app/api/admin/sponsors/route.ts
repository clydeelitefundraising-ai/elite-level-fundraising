...
export async function POST(req: NextRequest) {
  if (!await authed()) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  const { campaign_slug, name, url, tier, logo_url } = await req.json();
  ...
  const sponsor = await addSponsor({ campaign_slug: slug, name: name.trim(), url: url.trim(), tier, logo_url: logo_url?.trim() || null });
  logAuditEvent({
    ...
    new_value:     { name: name.trim(), url: url.trim(), tier, campaign_slug: slug, logo_url: logo_url?.trim() || null },
    ip_address:    ipOf(req),
